...
export default function PopUpTuto ({ children, nbrClick, trigerNumberClick, funcSendDataToParent, nbrTotalPopUp }) {
    ...
    return (
        <div ref={scope} className="absolute top-10 h-[88vh] w-[88vw] flex flex-row flex-wrap justify-center items-center rounded-3xl text-white backdrop-blur-xl shadow-[12px_12px_2px_1px] shadow-black/50 bg-green-military/50">
            {children}
            <ButtonNext funcOnClick={clickNext} className={"absolute h-[5vh] w-[5vw] border-2 rounded-3xl bottom-4 right-10 flex flex-row flex-wrap justify-center items-center cursor-pointer"} text={"Next"} />
            <ButtonPrev funcOnClick={clickPrev} className={"absolute h-[5vh] w-[5vw] border-2 rounded-3xl bottom-4 left-10 flex flex-row flex-wrap justify-center items-center cursor-pointer"} text={"Previous"} />
        </div>
    )
}